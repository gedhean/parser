...
-- Basic spatial indexes
CREATE SPATIAL INDEX IX_Spatial_Geometry ON SpatialTable (GeometryColumn) USING GEOMETRY_AUTO_GRID;
CREATE SPATIAL INDEX IX_Spatial_Geography ON SpatialTable (GeographyColumn) USING GEOGRAPHY_AUTO_GRID;

-- Spatial indexes with options
CREATE SPATIAL INDEX IX_Spatial_BoundingBox ON SpatialTable (GeometryColumn)
USING GEOMETRY_GRID
WITH (BOUNDING_BOX = (-180, -90, 180, 90));

CREATE SPATIAL INDEX IX_Spatial_GridLevels ON SpatialTable (GeometryColumn)
USING GEOMETRY_GRID
WITH (GRIDS = (LEVEL_1 = HIGH, LEVEL_2 = MEDIUM, LEVEL_3 = LOW, LEVEL_4 = LOW));

CREATE SPATIAL INDEX IX_Spatial_CellsPerObject ON SpatialTable (GeometryColumn)
USING GEOMETRY_GRID
WITH (CELLS_PER_OBJECT = 16);

-- Complex spatial index with multiple options
CREATE SPATIAL INDEX IX_Spatial_Complex ON dbo.SpatialTable (LocationColumn)
USING GEOMETRY_GRID
WITH (
...
-- Geography spatial index with schema-qualified table
CREATE SPATIAL INDEX IX_Geographic_Locations ON [GIS].[WorldLocations] ([Coordinates])
